...
# 👥 사용자 선택 메뉴가 화면 제일 위에 바로 나타납니다.
users = ["우진", "동탕"]
selected_user = st.selectbox("👤 학습자를 선택하세요", users)

# 👑 선택한 사용자에 따라서만 제목이 동적으로 뜹니다 (맨 위 고정 제목은 삭제!)
...
for i, r in enumerate(records):
    ...
    with col2:
        if is_english:
            ...
            st.write(f"<div style='color:#ff4d4d; font-size:20px; text-align:center; padding-top:10px;'>{stars}</div>", unsafe_allow_html=True)
